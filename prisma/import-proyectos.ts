 ...
 * import-proyectos.ts
 * Puebla: proyectos
 * Dependencias: Tercero (registradoPor)
 */
...
const CSV_PATH = path.join(os.homedir(), 'Desktop', 'tspine-csv', 'SistemaTspine1.0 - Proyectos.csv');
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (registradoPor && !tercerosByNombre.has(norm(registradoPor.toLowerCase()))) {
      incMap(registradoPorNR, registradoPor, id);
    }
  }
  ...
  console.log('\n[3/3] Truncando e importando proyectos...');
    ...
    try {
      await prisma.proyecto.create({
        data: {
          ...
          registradoPorId,
          proyecto:       getCol(row, 'PROYECTO')?.trim() || null,
          descripcion:    getCol(row, 'DESCRIPCION')?.trim() || null,
          fechaInicio:    parseDate(getCol(row, 'FECHA DE INICIO')),
          fechaFin:       parseDate(getCol(row, 'FECHA FIN')),
  ...
  console.log(`  → Total: ${importados} proyectos creados`);
  ...
  console.log(`  ⚠ Registrado Por s/resolver: ${registradoPorNR.size}`);
