/**
 * import-nombres-productos-especiales.ts
 * Puebla: nombres_productos_especiales
...
const CSV_PATH = path.join(os.homedir(), 'Desktop', 'tspine-csv', 'SistemaTspine1.0 - NombresProductosEspeciales.csv');
...
async function main() {
  ...
  console.log('  IMPORT NOMBRES PRODUCTOS ESPECIALES');
  console.log('═'.repeat(60));
  ...
  // ── [2/3] Truncar ─────────────────────────────────────────────────────────
  console.log('\n[2/3] Truncando nombres_productos_especiales...');
  await prisma.nombreProductoEspecial.deleteMany();
  console.log('  ✓ Tabla limpia');

  // ── [3/3] Importar ────────────────────────────────────────────────────────
  console.log('\n[3/3] Importando nombres de productos especiales...');
  ...
  for (const row of rows) {
    ...
    const nombreEspecial = getCol(row, 'NOMBREESPECIAL')?.trim() || null;
    ...
    try {
      await prisma.nombreProductoEspecial.upsert({
        where:  { id },
        update: {},
        create: {
          id,
