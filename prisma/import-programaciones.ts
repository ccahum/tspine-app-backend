...
function parseDate(raw: string): Date | null {
  ...
  // Parsear como ISO (YYYY-MM-DD) para que se guarde sin timezone
...
async function getOrCreateTercero(nombre: string, cache: Map<string, string>): Promise<string | null> {
  ...
  if (existing) { cache.set(nombre, existing.id); return existing.id; }
...
async function main() {
  ...
  // Caches para evitar duplicados
  const terceroCache = new Map<string, string>();

  // 1. Sedes
  const sedesSet = new Set<string>();
  rows.forEach(r => { if (r['SEDE']?.trim()) sedesSet.add(r['SEDE'].trim()); });
  ...
  // 2. Hospitales
  const hospitalesMap = new Map<string, string>(); // nombre → ciudad
  rows.forEach(r => {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      created++;
      if (created % 200 === 0) console.log(`  ${created} registros importados...`);
    } catch (err: any) {
